                        ...
                        <v-data-table :headers="headers"
                            ...
                            <template v-slot:no-data>
                                <div class="text-center py-8">
                                    ...
                                    <div class="text-h6 text-medium-emphasis mt-2">No task history records found</div>
                                    <v-btn prepend-icon="mdi-backup-restore" rounded="lg" text="Refresh" variant="text"
                                        border class="mt-4" @click="refreshHistory"></v-btn>
                                </div>
                            </template>
                        </v-data-table>
                    </v-sheet>
...
const logger = new DevelopmentLogger({ prefix: '[AdminTaskHistoryView]' });
...
const {
    ...
    load: refreshHistory,
    loading
} = useAsyncData<TaskHistoryExt[]>({
    fetchFunction: (signal) => tasksService.getAllTaskHistoriesExt(signal),
    loggerPrefix: '[AdminTaskHistoryView]'
});
